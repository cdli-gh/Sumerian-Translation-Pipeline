...
def main():
    ...
    if model_name == 'XLM' or model_name == 'MASS':
        if GPU==False:
    	    os.system(f'sh evalXLM.sh {output_dir}pipeline.txt {trans_path} {output_dir}trans_pipeline.txt')
        else:
    	    os.system(f'CUDA_VISIBLE_DEVICES=0 sh evalXLM.sh {output_dir}pipeline.txt {trans_path} {output_dir}trans_pipeline.txt')
